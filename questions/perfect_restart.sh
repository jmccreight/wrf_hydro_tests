...
echo -e "\e[7;49;32mperfect_restart.sh running candidate binary from restart (with $nCoresDefault cores).\e[0m"
...
if [[ $nSuccess -ne $nCoresDefault ]]; then
    echo Run run.candidate.restart failed.
    exit 2
fi
...
echo
echo -e "\e[0;49;32m-----------------------------------\e[0m"
echo -e "\e[7;49;32mQuestion: Perfect restarts?\e[0m".

#compare restart files
python3 $answerKeyDir/compare_restarts.py $domainRunDir/run.candidate.restart $domainRunDir/run.candidate \
    || { echo -e "\e[5;49;31mPerfect restarts: restart comparison failed.\e[0m"; exit 1; }
echo -e "\e[5;49;32mPerfect restart: restart comparison successful!\e[0m"
